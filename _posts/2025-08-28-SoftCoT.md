...
1. **CoT 추론의 두 단계**:
    1. **질문 Q**: 질문 $Q = [q_1, q_2, ..., q_|Q|]$ 가 주어지면, CoT 추론은 다음 두 단계로 문제를 해결.
    2. **합리화 단계 생성**: 질문에 따라 합리화 단계 $R = [r_1, r_2, ..., r_|R|]$ 목록을 자기회귀적으로 생성.
    3. **답변 생성**: 질문과 합리화 단계에 따라 답변 $A = [a_1, a_2, ..., a_|A|]$ 를 자기회귀적으로 생성.
