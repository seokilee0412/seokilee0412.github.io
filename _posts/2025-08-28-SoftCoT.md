...
1. **CoT 추론의 두 단계**:
    1. **질문 Q**: 질문 Q = [q1, q2, ..., q|Q|]가 주어지면, CoT 추론은 다음 두 단계로 문제를 해결.
    2. **합리화 단계 생성**: 질문에 따라 합리화 단계 R = [r1, r2, ..., r|R|] 목록을 자기회귀적으로 생성.
    3. **답변 생성**: 질문과 합리화 단계에 따라 답변 A = [a1, a2, ..., a|A|]를 자기회귀적으로 생성.
...
3. **답변 정확도 계산**:
    1. **답변 추출**: LLM이 생성한 답변 A에서 수동으로 정의된 규칙(예: 정규 표현식 매칭)을 사용하여 ˆA를 추출.
    2. **정확도 계산**: 추출된 ˆA를 정답 A와 비교하여 정확도를 계산.
