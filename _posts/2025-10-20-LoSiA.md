...
2. $W - W' \approx \eta \cdot \text{(선택되지 않은 gradient)}$

    1. 선택 안된 weight는 업데이트 안 되니까 그대로 → 오차 발생
...
- 결론
    - MSE upper bound는 **선택하지 않은 weight의 gradient 총량**에 비례 → gradient가 큰 weight를 놓치면, MSE도 커짐
